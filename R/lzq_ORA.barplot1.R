#' @export
lzq_ORA.barplot1 <- function (enrich.obj, x = "GeneRatio", show.term.num = 15, color.by = "p.adjust", 
    colors = c("#003c30", "#01665e", "#35978f", "#80cdc1", "#c7eae5", 
        "#f6e8c3", "#dfc27d", "#bf812d", "#8c510a", "#543005"), 
    color.title = color.by, bar.width = 0.6, add.bar.border = F, 
    y.label.position = "right", title = NULL, legend.position = "right", 
    theme.plot = theme_bw(base_rect_size = 1.5), use.Chinese = F, 
    appid = "20231122001888718", key = "5GpDqe8F3pmXfnOkEKGQ") 
{
    ...
    suppressWarnings(ggplot(dd, aes_string(x, "Description", 
        fill = color.by)) + geom_bar(stat = "identity", width = bar.width, 
        color = ifelse(add.bar.border, "black", NA)) + theme.plot + 
        labs(fill = color.title, x = x.lab, y = NULL, title = title) + 
        scale_fill_gradientn(colours = colors) + scale_y_discrete(labels = Hmisc::capitalize, 
        position = y.label.position) + theme(axis.text.x = element_text(size = 10, 
        colour = "black"), axis.title.x = element_text(size = 13, 
        colour = "black", face = "bold"), axis.text.y = element_text(size = 13, 
        colour = "black"), axis.title = element_blank(), panel.background = element_blank(), 
        legend.text = element_text(size = 11, colour = "black"), 
        legend.title = element_text(size = 13, colour = "black", 
            face = "bold"), legend.background = element_blank(), 
