#' @export
lzq_GSEA.dotplot1 <- function (enrich.obj, type = "neg", show.term.num = 15, Selct.P = "FDR", 
    cutoff.P = 0.05, colors = c("#003c30", "#01665e", "#35978f", 
        "#80cdc1", "#c7eae5", "#f6e8c3", "#dfc27d", "#bf812d", 
        "#8c510a", "#543005"), size.range = c(3, 8), y.label.position = "right", 
    ...
    use.Chinese = F, appid = "20231122001888718", key = "5GpDqe8F3pmXfnOkEKGQ") 
{
    ...
    ggplot(r, aes(sig, Description, fill = abs(NES), size = abs(NES))) + 
        geom_point(shape = 21, color = "black") + labs(size = color.title, 
        fill = color.title, x = x.lab, y = NULL, title = title) + 
        ...
        legend.title = element_text(size = 13, colour = "black", 
            ...
            colour = "black")) + guides(fill = guide_legend())
}
